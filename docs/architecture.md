...
**The RC null is cash, not buy-and-hold.** White (2000) uses zero return as the benchmark. A strategy can have a small RC p-value (beats cash) while still underperforming B&H. Both are reported and described separately in the output. To test against B&H directly, you'd resample active returns (strategy minus B&H) rather than raw returns - that's not currently implemented.
